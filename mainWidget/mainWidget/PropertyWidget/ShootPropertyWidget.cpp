...
void ShootPropertyWidget::initWidget()
{
	...
	QStringList labels = { "属性","测试项目","撞击速度", "撞击角度", "子弹型式", "子弹直径","子弹硬度","温度观测点数量","应力观测点数量","风速","壳体破损判断","推进剂发火温度判断","推进剂发火压力判断" };
	...
	// 设置列宽度
	QTableWidgetItem* colimnItem = m_tableWidget->item(11, 1);
	...
	// 单位
	QStringList unitLabels = { " ", " ", "m/s", "°", " ", "mm", " ", "个", "个", "m/s", "", "", ""};
	...
	QTableWidgetItem* bulletTypeValueItem = new QTableWidgetItem("穿甲弹");
	bulletTypeValueItem->setTextAlignment(Qt::AlignCenter); // 文本居中
	bulletTypeValueItem->setFlags(bulletTypeValueItem->flags() & ~Qt::ItemIsEditable); // 不可编辑

	QTableWidgetItem* bulletDiameterValueItem = new QTableWidgetItem("12.7");
	bulletDiameterValueItem->setTextAlignment(Qt::AlignCenter); // 文本居中
	bulletDiameterValueItem->setFlags(bulletDiameterValueItem->flags() & ~Qt::ItemIsEditable); // 不可编辑

	QTableWidgetItem* bulletHardness = new QTableWidgetItem("刚体");
	...
	m_tableWidget->setItem(4, 2, bulletTypeValueItem);
	m_tableWidget->setItem(5, 2, bulletDiameterValueItem);
	m_tableWidget->setItem(6, 2, bulletHardness);
	m_tableWidget->setItem(7, 2, temperatureNumValueItem);
	m_tableWidget->setItem(8, 2, overpressureNumValueItem);
	m_tableWidget->setItem(9, 2, windSpeedValueItem);
	m_tableWidget->setItem(10, 2, new QTableWidgetItem(""));
	m_tableWidget->setItem(11, 2, new QTableWidgetItem(""));
	m_tableWidget->setItem(12, 2, new QTableWidgetItem(""));
	m_tableWidget->item(10, 2)->setBackground(QBrush(QColor(230, 230, 230)));
	m_tableWidget->item(11, 2)->setBackground(QBrush(QColor(230, 230, 230)));
	m_tableWidget->item(12, 2)->setBackground(QBrush(QColor(230, 230, 230)));
