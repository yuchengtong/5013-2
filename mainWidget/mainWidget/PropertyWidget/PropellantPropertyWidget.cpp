...
PropellantPropertyWidget::PropellantPropertyWidget(QWidget* parent)
		...
		if (row != 0)
		{
			...
			while (parent) {
				...
				if (gfParent)
				{
					...
					valueItem->setFlags(valueItem->flags() & ~Qt::ItemIsEditable); // 不可编辑
					valueItem->setBackground(QBrush(QColor(230, 230, 230)));
					materialTableWid->setItem(2, 2, valueItem);
					break;
				}
				else
				{
...
void PropellantPropertyWidget::setMasterialData(QString model)
{
	...
	{
		info.name = "ROTL-1";
		info.type = "HTPE 高能推进剂";
		info.density = 1820.0;
		info.specificHeatCapacity = 1380.0;
		info.thermalConductivity = 0.24;
	}
	else if (model == "YJ-20")
	{
		info.name = "RBUOL-1";
		info.type = "PBAA 聚丙烯酸酯推进剂";
		info.density = 1750.0;
		info.specificHeatCapacity = 1320.0;
		info.thermalConductivity = 0.21;
	}
	else if (model == "YJ-91A")
	{
		info.name = "THL";
		info.type = "HTPE 复合推进剂";
		info.density = 1780.0;
		info.specificHeatCapacity = 1350.0;
		info.thermalConductivity = 0.22;
	}
	else if (model == "CJ-20A")
	{
		info.name = "RBOL-1";
		info.type = "NEPE 硝酸酯增塑聚醚推进剂";
		info.density = 1850.0;
		info.specificHeatCapacity = 1420.0;
		info.thermalConductivity = 0.26;
	}
	info.isChecked = true;
	ins->SetPropellantPropertyInfo(info);
	...
	while (parent) {
		...
		if (gfParent)
		{
			...
			auto logWidget = gfParent->GetLogWidget();
			auto textEdit = logWidget->GetTextEdit();
			QString text = timeStr + "[信息]>开始导入药液物性材料数据";
			textEdit->appendPlainText(text);
			logWidget->update();
