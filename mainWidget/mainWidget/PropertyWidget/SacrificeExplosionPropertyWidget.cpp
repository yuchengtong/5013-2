...
SacrificeExplosionPropertyWidget::SacrificeExplosionPropertyWidget(QWidget* parent)
	:BasePropertyWidget(parent)
{
	initWidget();
}

void SacrificeExplosionPropertyWidget::initWidget()
{
	...
	ammunitionQuantityValueItem->setTextAlignment(Qt::AlignCenter); // 文本居中
	ammunitionQuantityValueItem->setFlags(ammunitionQuantityValueItem->flags() & ~Qt::ItemIsEditable); // 不可编辑

	QTableWidgetItem* firedNumValueItem = new QTableWidgetItem("90");
	firedNumValueItem->setTextAlignment(Qt::AlignCenter); // 文本居中
	firedNumValueItem->setFlags(firedNumValueItem->flags() & ~Qt::ItemIsEditable); // 不可编辑
	...
	m_tableWidget->setItem(2, 2, distanceValueItem);
	m_tableWidget->setItem(3, 2, ammunitionQuantityValueItem);
	m_tableWidget->setItem(4, 2, firedNumValueItem);
