		...
		if (gfParent)
		{
			...
			connect(calculateWorker, &ForwardDesignWorker::FrameCalculated, this,
				[=](int frameIndex) {
					...
					APISetNodeValue::SetInForwardDesignResult(occView, nodeValues, frameIndex - 1);

					auto inForwardPropertyInfo2 = ins->GetInForwardPropertyInfo();
					...
					// 第一帧时创建色条，后续帧只更新标题
					if (inForwardPropertyInfo2.m_ColorScale.IsNull())
					{
						double min_value = 0;
					...
					{
						inForwardPropertyInfo2.m_ColorScale->SetTitle(newTitle);
						context->Redisplay(inForwardPropertyInfo2.m_ColorScale, true);
					}
			...
			connect(calculateWorker, &ForwardDesignWorker::WorkFinished, this,
				[=](bool success, const QString& msg) mutable {
					if (success)
					{
