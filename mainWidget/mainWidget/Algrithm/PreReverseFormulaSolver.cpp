...
double PreReverseFormulaSolver::parseFactor(const QString& factor, double A, double B, double C, double D, double E, double F)
{
...
double PreReverseFormulaSolver::parseTerm(const QString& term, double A, double B, double C, double D, double E, double F)
{
    ...
    for (QString f : factors)
    {
        ...
        val *= parseFactor(f, A, B, C, D, E, F);
    }

    return val;
}

double PreReverseFormulaSolver::evaluate(const QString& expr, double A, double B, double C, double D, double E, double F)
{
    QString s = expr;
    s.replace("-", "+-");
    ...
    for (QString term : terms)
    {
        ...
        total += parseTerm(term, A, B, C, D, E, F);
    }
    ...
    double A, double B, double C, double D, double E, double F, char uv)
{
    ...
    {
        if (uv == 'A') return evaluate(formula, x, B, C, D, E, F);
        if (uv == 'B') return evaluate(formula, A, x, C, D, E, F);
        if (uv == 'C') return evaluate(formula, A, B, x, D, E, F);
        if (uv == 'D') return evaluate(formula, A, B, C, x, E, F);
        if (uv == 'E') return evaluate(formula, A, B, C, D, x, F);
        if (uv == 'F') return evaluate(formula, A, B, C, D, E, x);
        return 0.0;
    };
...
{
    vector<double> res;
    double ans = findBestMatch(formula, targetValue, A, B, C, D, E, F, unknownVar);
    res.push_back(ans);
